        ...
        (config, services) => ({
            ...
            execute: async (getSummaryAccept, propName) => {
                ...
                try {
                    ...
                    let friends = propName.friend.parent.multiple.map(f => f.baseWord);
                    let preFriends = propName.friend.parent.multiple
                        .filter(w => w.preposition.origWord == 'pre')
                        .map(f => f.baseWord);
                    if (preFriends.length) friends = preFriends;
